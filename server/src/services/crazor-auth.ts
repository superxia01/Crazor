...
function sign(data: string): string {
  // Bun's built-in crypto
  const key = new Bun.CryptoKey(JWT_SECRET, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign'])
  // @ts-ignore — Bun's subtle crypto
  const sig = crypto.subtle.signSync('HMAC', key, new TextEncoder().encode(data))
