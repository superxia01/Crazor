...
test("customer delivery records are a real business module across API, UI, context, and smoke checks", () => {
  ...
  assert.ok(
    serverIndex.includes("app.get('/api/crazor/deliveries'") &&
      ...
      serverIndex.includes("app.post('/api/crazor/projects/:id/attachments'") &&
      serverIndex.includes("app.post('/api/crazor/deliveries/:id/attachments'") &&
      serverIndex.includes("delivery_attachment") &&
    ...
    "server should route delivery CRUD, contact kickoff, project/delivery attachments, analytics, read protection, and readiness checks",
  )

  assert.ok(
    permissionsSource.includes('"delivery:*"') &&
      permissionsSource.includes('"delivery:read"') &&
      permissionsSource.includes('"project_attachment"') &&
      permissionsSource.includes('"delivery_attachment"'),
    "role policies should protect delivery reads, writes, and attachment operations",
  )
  ...
  assert.ok(
    appInnerSource.includes('import("@/DeliveriesView")') &&
      ...
      deliveriesConfigSource.includes("DeliveryAttachmentsPanel") &&
      deliveriesConfigSource.includes("/attachments") &&
    ...
    "frontend should expose an editable delivery management workspace, contact kickoff path, and delivery attachments",
  )
  ...
  assert.ok(
    desktopSmokeSource.includes("/api/crazor/deliveries") &&
      ...
      dockerSmokeSource.includes("/api/crazor/deliveries/${encodeURIComponent(delivery.id)}/attachments") &&
      dockerSmokeSource.includes("delivery_attachment") &&
      dockerSmokeSource.includes("project_attachment") &&
      dockerSmokeSource.includes("types: \"contact,project,task,delivery") &&
      dockerSmokeSource.includes("entity === \"delivery\""),
    "customer delivery and Docker smoke tests should verify the delivery kickoff and attachment chain",
  )
})
