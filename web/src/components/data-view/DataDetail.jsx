...
export default function DataDetail({ open, onClose, item, config, onReload, onItemUpdate }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={cn("max-h-[85vh] overflow-auto", config.detailMaxWidth || "max-w-lg")}>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3">
            ...
            <div>
              ...
              {config.detailSubtitle && (
                <DialogDescription className="text-[12px]">{config.detailSubtitle(item)}</DialogDescription>
              )}
            </div>
          </DialogTitle>
        </DialogHeader>
