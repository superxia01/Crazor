...
也可以在 GitHub Actions 的“构建客户桌面安装包”工作流中输入客户名称和服务地址，由远端构建机生成安装包 artifact。手动触发时可选择 `handoff_mode=offline`、`online-entrypoints` 或 `online-chat`；在线模式会让 runner 连接托管后端并把验收结果写入报告。若后端启用了客户访问码或登录 token，可提前配置仓库 Secrets：`CRAZOR_CUSTOMER_ACCESS_CODE`、`CRAZOR_CUSTOMER_LOGIN_TOKEN`、`CRAZOR_SMOKE_TOKEN`。
