...
- **国际化** — 简体中文、English
- **响应式** — 手机/平板/PC 自适应布局
- **桌面客户端** — Tauri 桌面应用，支持 macOS / Windows，客户包启动前会检查托管后端、登录、对话网关、模型配置和知识库状态
...
如果要部署到远端 Docker 主机，使用固定部署入口；它会在远端保留 `shared/data` 相对数据目录，把代码发布到 release 目录并切换 `current` 软链接：
...
./scripts/hermes customer-build .env.customer windows-current
...
也可以在 GitHub Actions 的“构建客户桌面安装包”工作流中输入客户名称和服务地址，由远端构建机生成安装包 artifact。PR 会默认同时构建 `macos-current` 和 `windows-current` 两个平台；手动触发时可选择单个平台，并可选择 `handoff_mode=offline`、`online-entrypoints` 或 `online-chat`；在线模式会让 runner 连接托管后端并把验收结果写入报告。若后端启用了客户访问码或登录 token，可提前配置仓库 Secrets：`CRAZOR_CUSTOMER_ACCESS_CODE`、`CRAZOR_CUSTOMER_LOGIN_TOKEN`、`CRAZOR_SMOKE_TOKEN`。
安装包构建完成后会自检前端产物是否嵌入目标服务地址、Tauri bundle 是否包含目标平台安装包，并生成 `desktop/src-tauri/target/release/customer-delivery/` 精简交付目录。该目录包含客户安装包、`crazor-start-here.md`、`crazor-delivery-manifest.json`、`crazor-delivery-checksums.txt`，CI 与验收命令还会写入不含访问码明文和 API Key 明文的 `crazor-handoff-report.md`，报告只记录模型连接变量名或本地模型来源，并归档后端交付自检项，不包含 Tauri 构建辅助文件。`crazor-start-here.md` 会明确网页版入口、桌面安装包、后端自检地址和验收文件，避免客户拿到 artifact 后不知道从哪里开始。Manifest 会记录客户名、服务地址、交付指纹、平台（如 `macos-current` / `windows-current`）、构建来源、安装文件清单、文件大小和 SHA256，客户端设置页也会显示交付指纹、构建版本和构建时间，便于客户现场验收与后续问题追溯。
CI 还会运行安装器烟测：macOS 会校验并只读挂载 DMG，Windows 会用系统安装器行政解包 MSI 并确认可执行文件存在。默认签名问题以警告呈现；正式客户包在手动触发工作流时把 `installer_trust_mode` 设为 `require-trusted`，CI 会设置 `CRAZOR_REQUIRE_INSTALLER_TRUST=1`，把 macOS Gatekeeper / Windows Authenticode 校验提升为硬门槛。
正式 macOS 包可配置仓库 Secrets：`APPLE_CERTIFICATE`、`APPLE_CERTIFICATE_PASSWORD`、`APPLE_SIGNING_IDENTITY`、`APPLE_ID`、`APPLE_PASSWORD`、`APPLE_TEAM_ID`，或使用 App Store Connect API 的 `APPLE_API_KEY`、`APPLE_API_ISSUER`、`APPLE_API_KEY_P8`。正式 Windows 包可配置 `WINDOWS_CERTIFICATE`、`WINDOWS_CERTIFICATE_PASSWORD`、`WINDOWS_CERTIFICATE_THUMBPRINT`，并按需配置仓库 Variables：`WINDOWS_DIGEST_ALGORITHM`、`WINDOWS_TIMESTAMP_URL`、`TAURI_WINDOWS_SIGNTOOL_PATH`。
交付目录会通过 `scripts/verify-customer-delivery.mjs` 自动校验；下载 CI artifact 后也可以运行同一脚本复验客户起步说明、manifest、checksum 和安装包 SHA256。CI 会在生成验收报告后再次校验最终交付目录，再上传 artifact。
...
如只想快速验证托管后端、网页版统一入口及其前端静态资源、交付身份、登录门禁、客户/项目/任务/知识库/附件策略等业务入口和对话能力入口，也可以单独运行客户桌面远程烟测：

```bash
./scripts/hermes desktop-smoke "客户名称" "https://crazor.example.com"
```

默认会实际调用一次非流式对话并校验返回文本；如后端启用了客户访问码登录，可设置 `CRAZOR_DESKTOP_SMOKE_ACCESS_CODE=<访问码>` 自动登录后继续验证业务入口和对话；如只想快速检查入口，可设置 `CRAZOR_DESKTOP_SMOKE_SKIP_LIVE_CHAT=1`。
