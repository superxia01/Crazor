...
| 分支 | 写者 | 目标 | 占用的表/模块 | 状态 |
|------|------|------|--------------|------|
| feature/m0-team-collaboration | Claude | 团队多用户改造 | team_members/users/invite_codes/11业务表加列；auth/permissions/mcp | 待 Codex 审计 |
| codex/m0-collaboration-audited | Codex | M0 审计 | 同上（只读审计） | 进行中 |
| feature/m1-event-bus | Claude | 事件总线+WS | event-bus.ts(新)/index.ts WS 路由/DataView | 待审计（依赖 M0 合并） |
