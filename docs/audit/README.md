...
客户 Case 的最小工作台也已接入并完成深链路验证：客户详情可以新增跟进记录、新建需求文档、搜索并打开客户文档、上传/下载/删除客户附件、登记成交、建立渠道转介绍，从客户生成项目机会，并从客户项目直接拆解任务。
...
- 客户 Case 已打通第一层业务深链路、项目任务联动、客户文档搜索跳转和附件归档；下一步要继续补权限边界、附件类型策略和跨模块提醒。
