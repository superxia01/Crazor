...
交付烟测已沉淀为可重复执行脚本：`./scripts/hermes smoke` 会通过 Web 统一入口创建并清理临时客户、文档、附件、渠道、流水、项目、任务、内容和 token，验证后端健康、Hermes 状态代理、身份 token、业务只读边界、客户 Case、跨模块提醒、内容发布、分析概览和审计日志。
...
| 内容发布、指标回收和知识正文链路 | 通过 |
| 分析概览与审计日志 | 通过 |
| 临时数据清理 | 通过，客户、文档、附件、渠道、流水、项目、任务、内容和临时身份均已清理 |
...
- 每次影响 Docker 交付、核心 API、身份权限、客户 Case、文档、附件、项目任务、内容发布或审计日志时，必须运行 `./scripts/hermes smoke`。
