...
### P1：客户 Case 已补深链路和项目任务联动，附件归档和搜索跳转仍待补

**现象**

- 客户基础资料、跟进记录、需求文档、文档打开编辑、成交登记、渠道转介绍、项目机会、项目任务拆解已经可以在客户详情完成第一层深链路。
- 客户文档、渠道转介绍、项目机会、任务拆解写入均已通过统一入口 API 验证，并进入 `audit_logs`。
- 后端已支持 `/api/crazor/tasks?contact_id=:id` 按客户读取关联项目下任务，前端客户详情会在项目机会下展示任务，并可直接从项目拆任务。
- 当前还没有把客户附件归档、客户文档搜索跳转全部纳入客户详情。

**影响**

客户维护到项目任务的核心链路已经可以演示，但附件归档和文档搜索仍可能产生上下文切换。

**建议**

下一步补客户附件归档入口、客户文档搜索结果跳转，并继续验证这些动作的审计记录。
