...
### P0：强制写入认证、角色级写入上限与敏感只读保护已补，完整登录态和审批仍未完成
...
- 但当前还没有完整登录态、业务只读接口保护策略、关键操作审批，以及可配置到页面策略层的细粒度 RBAC 矩阵。

**影响**

现在可以回答“这次写入或敏感读取是否有可信身份、足够 token scope，并且未突破角色上限”，也能审计无 token 与越权尝试；但页面级登录态、业务只读接口保护、关键审批和完整权限策略管理尚未形成。

**建议**

下一步补完整登录态、业务只读接口保护策略、可配置的接口级 RBAC 策略、关键写入操作审批和高风险操作二次确认。

### P1：客户 Case 已补深链路、项目任务联动、文档搜索、附件策略和附件预览

**现象**

- 客户基础资料、跟进记录、需求文档、文档打开编辑、客户文档正文搜索、附件策略校验、附件上传/下载/删除、文本/图片附件预览、成交登记、渠道转介绍、项目机会、项目任务拆解已经可以在客户详情完成第一层深链路。
- 客户文档、附件归档、渠道转介绍、项目机会、任务拆解写入均已通过统一入口 API 验证，并进入 `audit_logs`。
...
客户维护到项目任务、需求文档和附件归档的核心链路已经可以演示，客户详情不再需要跳出统一入口处理文档检索、附件归档和常见附件预览。

**建议**

下一步补关键客户动作提醒、客户级权限边界、附件病毒扫描/敏感信息扫描和更多格式预览。
...
### P1：内容正文与追踪记录跳转链路已补，内容复盘体验仍需核验

**现象**

- 数据架构要求 `content_pieces.doc_id` 关联知识库正文。
- 内容详情已接入“创建正文 / 打开正文 / 保存正文”：没有 `doc_id` 时会创建知识库正文并回填内容作品，已有 `doc_id` 时可直接打开并编辑正文。
- `POST /api/crazor/docs/:scope/notes` 已支持 `content`，创建正文不再只生成空笔记。
- 临时内容作品和正文文档已通过统一入口端到端验证，并进入 `audit_logs`。
- 当前还需要继续核验内容检索、发布复盘、指标回收和正文搜索跳转体验。

**影响**

内容生产从追踪记录到正文文档已经能闭合，但内容复盘和跨文档检索仍可能需要上下文切换。

**建议**

下一步补内容搜索结果跳转、发布复盘模板和指标回收动作的端到端验证。
...
1. 补完整登录态、业务只读接口保护策略、角色权限矩阵细化、关键写入审批和高风险操作二次确认。
2. 补关键客户动作提醒、客户级权限边界、附件病毒扫描/敏感信息扫描和更多格式预览。
3. 收敛 Agent Provider 能力抽象，减少 UI 对 Hermes 私有概念的硬依赖。
4. 补内容搜索结果跳转、发布复盘模板和指标回收动作验证。
...
| 内容正文关联烟测 | 临时内容作品创建后可创建正文、回填 `doc_id`、打开保存正文，`doc_note` 和 `content_piece` 审计均通过 |
