...
### P1：客户交付形式只能作为协作 MVP，不能作为正式交付系统

**现象**

- 当前交付链路由客户、需求文档、成交流水、项目、任务、交付记录、客户/项目/交付附件和知识库指南组合而成。
- 知识库会初始化 `20-业务流程/40-产品交付`、`30-素材资产/50-报价方案`、`30-素材资产/60-合同`，并带有 `产品交付-填写指南.md`、`交付标准.md` 等模板。
- 客户详情可从客户生成项目机会、拆任务、创建需求文档、上传附件，并一键启动交付生成交付记录和计划文档；Agent 也可通过 MCP 创建文档、项目和任务。
- 已新增独立交付记录实体，包含交付阶段、验收状态、交付物、风险、负责人、客户/项目关联，并接入 REST、前端交付管理页、分析概览、Unified Context、审计日志和 Docker 烟测。
- 项目和交付记录已支持附件归档分类，交付材料、合同、课件、交付包和验收材料可以挂到更明确的业务节点。
- 仍缺客户签收、客户侧查看入口、交付导出包、附件扫描和客户级权限边界。
- 成交流水的回款状态也没有和项目交付状态、验收状态强关联。

**影响**

这套形态可以用于内部轻量协作和客户演示，但如果直接对外承诺为“客户交付管理系统”，会在验收、归档、权限、交付物追踪和争议复盘上不够可靠。

**建议**

短期演示时明确称为“客户交付协作 MVP”。下一步把客户侧查看、签收归档、交付导出包、附件扫描、项目任务里程碑和客户级权限边界串成正式闭环。
...
2. 补客户侧交付查看、签收归档、交付导出包和项目任务里程碑。
3. 补客户级权限边界、更细的提醒规则配置、附件病毒扫描/敏感信息扫描和更多格式预览。
4. 收敛 Agent Provider 能力抽象，减少 UI 对 Hermes 私有概念的硬依赖。
5. 补外部平台发布回执、指标自动采集、内容级权限边界和发布前审批。
6. 持续梳理剩余占位工具、旧 mock 模块和直接外部 fetch 工具。
...
| 项目和交付附件没有归档入口 | 已修复，项目和交付记录已接入附件上传、列表、预览、下载、删除和分类元数据，并记录 `project_attachment` / `delivery_attachment` 审计 |
...
| 项目/交付附件归档烟测 | 项目附件和交付附件可上传、分类、列表读回、预览、下载，并进入 `project_attachment` / `delivery_attachment` 审计 |
...
| 自动交付烟测脚本 | `node --check scripts/crazor-smoke.mjs`、`bash -n scripts/hermes`、`./scripts/hermes smoke`、`./scripts/hermes smoke-strict` 均通过；严格模式匿名 REST/MCP 写入被拒绝；MCP initialize/tools/list/create_contact/get_task_reminders 通过；临时客户、MCP 客户、文档、附件、渠道、流水、项目、任务、交付记录、项目/交付附件、内容和身份已清理 |
...
| 远程最新代码同步与客户交付形式审计 | `git fetch --all --prune` 已同步 `origin/master@0c6a266`、`origin/main@83b7dee` 和 `v1.0.0`；Hermes 版本接口最新修复已纳入当前分支；运行态 `/api/hermes/version` 返回 `version=0.14.0`、`release_date=2026.5.16`、`gateway_running=true`；客户交付形式审计已记录到 `docs/audit/customer-delivery-form-review.md` |
