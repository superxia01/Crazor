...
| 项目看板 | `projects` | `/api/crazor/projects`、`/api/crazor/tasks`、`/api/crazor/task-reminders` | `projects`、`tasks` | 项目/任务 MCP tools + `get_task_reminders` | 项目创建、任务创建、任务拖拽/删除、任务到期提醒读取和处理可用；可支撑客户交付协作 MVP，但缺少正式交付单、里程碑、验收和交付物结构化字段；项目编辑归档待补 |
...
| 客户交付材料 | `knowledge` + `projects` + `tasks` | `/api/crazor/docs/knowledge/*`、`/api/crazor/projects`、`/api/crazor/tasks`、`/api/crazor/transactions` | Markdown Vault、`projects`、`tasks`、`transactions` | 文档 MCP tools + 项目/任务 MCP tools | 有产品交付、报价、合同、交付标准等知识库指南，并可从客户生成项目和任务；当前不是独立交付模块，缺交付记录、验收、交付物和客户侧视图 |
...
| 集成 / 连接器 | `integrations` | `/api/env`、`/api/env/:key/reveal` | Hermes Dashboard env 配置 | Provider 环境变量 | 可保存、读回、保留和清除预设/自定义连接器凭证；已兼容对象型 Dashboard metadata；状态显示为“凭证完整/部分填写”，避免误判为外部业务已连接；Get 笔记真实 API 适配完成前不暴露占位同步 MCP 工具；真实回调、同步任务和外部平台连通性测试待补 |
