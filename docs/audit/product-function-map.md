...
graph TD
  ...
  Server --> Permission["写入认证 / Token Scope / 角色上限"]
...
| 团队身份与接入凭证 | `teamops` 协作审计 | `/api/crazor/identity/me`、`/api/crazor/identity/members`、`/api/crazor/identity/tokens` | `team_members`、`actor_tokens.scopes` | REST/MCP 可通过 token 派生 actor，校验 scope，并受角色写入上限约束 | 最小 UI 与 API 可用；当前访问 token、token scope、强制写入认证和角色级写入上限已接入，完整登录态、只读保护、审批和细粒度 RBAC 待补 |
| 操作审计 | `teamops` 协作审计 | `/api/crazor/audit-logs` | `audit_logs` | MCP 写入、无 token、scope 越权和角色越权拒绝自动记录 | REST/MCP 写入、`missing-token deny_*`、scope 越权和角色越权 `deny_*` 审计可用；审批流待补 |
