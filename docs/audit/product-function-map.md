...
| 集成 / 连接器 | `integrations` | `/api/env`、`/api/env/:key/reveal` | Hermes Dashboard env 配置 | Provider 环境变量 | 可保存、读回、保留和清除预设/自定义连接器凭证；已兼容对象型 Dashboard metadata；状态显示为“凭证完整/部分填写”，避免误判为外部业务已连接；真实回调、同步任务和外部平台连通性测试待补 |
