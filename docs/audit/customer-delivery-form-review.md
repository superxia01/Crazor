# 客户交付形式可靠性审计

> 更新日期：2026-06-03
> 结论范围：基于当前 `codex/docker-hermes-mvp` 分支和已 fetch 的远程最新代码。

## 结论

当前客户交付形式已经具备内部轻量协作和客户演示的最小闭环，但还不应直接包装成完整客户交付门户。

可靠的部分：

- 客户可以进入 CRM，并能关联跟进、需求文档、附件、成交流水、渠道来源、项目机会和项目任务。
- 知识库首启会创建 `20-业务流程/40-产品交付`、`30-素材资产/50-报价方案`、`30-素材资产/60-合同` 等目录。
- `产品交付-填写指南.md`、`交付标准.md`、报价和合同指南能作为交付材料规范入口。
- Agent 可通过 MCP 发现知识库目录，创建文档、创建项目、拆任务、登记成交和读取任务提醒。
- Docker 烟测覆盖客户、文档、附件、成交、项目、任务、交付记录和 Agent MCP 写入链路。

不可靠的部分：

- 交付记录实体已补齐，但客户侧查看、签收确认、导出包和交付归档动作还没有形成客户可访问闭环。
- 项目任务仍缺少正式里程碑、阻塞原因、依赖关系和版本交付记录。
- 任务只有 `todo / doing / done`，无法表达交付里程碑、客户验收、返工、阻塞、依赖和版本交付。
- 成交流水的回款进度没有和项目交付状态、验收状态形成强关联。
- 附件已可挂到客户、项目和交付记录，但还缺少扫描、签收归档和客户侧下载包。
- 交付权限仍是业务域级 token，不是客户级、项目级或交付级权限。

## 当前链路

```mermaid
flowchart LR
  Lead["客户线索"] --> Follow["跟进记录"]
  Lead --> DemandDoc["需求文档"]
  Lead --> Deal["成交流水"]
  Lead --> Project["项目机会"]
  Project --> Task["项目任务"]
  DemandDoc --> Vault["知识库"]
  Lead --> Attachment["客户附件"]
  Project --> ProjectAttachment["项目附件"]
  Project --> Delivery["交付记录"]
  Delivery --> DeliveryAttachment["交付附件"]
  Vault --> DeliveryGuide["产品交付指南"]
```

这条链路能证明“客户从线索到项目、交付记录和交付材料”的内部协作闭环，但还不能证明“客户交付物从约定到客户签收”的正式闭环。

## 建议优先级

P0：不要把当前形态包装成正式客户交付系统。演示时应表述为“客户交付协作 MVP”。

已完成：新增交付记录实体，包含 `contact_id`、`project_id`、`delivery_type`、`stage`、`owner`、`start_date`、`due_date`、`acceptance_status`、`deliverables`、`risks`、`handover_doc_id`，并接入前端交付管理、REST、审计、分析和 Unified Context。

已完成：客户详情可一键启动交付，自动复用或创建客户项目，生成交付记录，并把交付计划文档写入 `20-业务流程/40-产品交付/<客户>`。

已完成：附件支持挂到项目和交付记录，并可用需求材料、合同、课件、交付包、验收材料等分类区分。

P2：项目任务增加里程碑和阻塞原因，任务完成不等于客户验收。

P2：补客户级 / 项目级 / 交付级权限，避免多人协作时跨客户误读。

## 可演示边界

可以演示：

- 客户录入、跟进、需求文档、附件归档。
- 成交登记后生成项目机会。
- 项目拆任务，Agent 读取任务提醒。
- 知识库里维护交付标准、报价和合同指南。

暂不建议演示为已完成：

- 客户验收和签收。
- 客户侧查看、下载交付包和签收归档。
- 合同 / 报价单自动生成。
- 项目交付阶段和回款联动。
- 客户级权限隔离。
