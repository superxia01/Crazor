...
CI 会额外执行安装器烟测：macOS runner 校验并挂载 DMG，Windows runner 使用 `msiexec /a` 解包 MSI 并确认可执行文件存在。默认未签名或未公证只作为警告；正式客户交付时设置 `CRAZOR_REQUIRE_INSTALLER_TRUST=1`，让 macOS Gatekeeper 或 Windows Authenticode 未通过时直接失败。
