...
如果通过 GitHub Actions 生成客户包，但后端只在客户局域网内可访问，手动触发工作流时把 `preflight_mode` 设为 `warn` 或 `skip`；如果 runner 能访问客户后端，再设为 `strict`。工作流的 `platform` 会原样写入交付 manifest，例如 `macos-current` 或 `windows-current`，用于区分客户实际安装包平台。
